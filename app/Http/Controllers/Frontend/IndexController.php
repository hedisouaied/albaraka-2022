...
class IndexController extends Controller
{
    public function home()
    {
        ...
        $event = Event::where(['status' => 'active'])->orderBy('id', 'DESC')->first();
        $references = Reference::where(['status' => 'active'])->inRandomOrder()->limit('6')->get();

        $header_home = "accueil";

        return view('frontend.index', compact(['banners','domaines','partenaires','event','references','header_home']));
    }
    ...
    public function blogDetail($slug)
    {
        ...
        $title_page = $blog->title;
            ...
            return view('frontend.pages.blog-details', compact(['blog', 'meta', 'title_page','related_blogs','domaines']));
        } else {
            return view('errors.404');
        }
    }
